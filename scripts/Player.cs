...
public partial class Player : RigidBody2D
{
    ...
    /// </summary>
    [Export(PropertyHint.Range, "-2,2,1,or_greater,or_less")] private float _q = -2f;
    ...
    public override void _PhysicsProcess(double delta)
    {
        ...
        float speedInMovementDirection = LinearVelocity.Dot(movementDirection);
        ...
        {
            // Fancy curve (See above desmos link and doc comment for _q)
            case 0:
                speedFactor = Mathf.Clamp(speedInMovementDirection / _maxSpeed, -1f + Mathf.Epsilon, 1f);

                if (_q != 0f)
                {
                    float inner = Mathf.Exp(2f * _q) - 1f;
                    float numerator = Mathf.Log(inner / (Mathf.Exp((speedFactor + 1f) * _q) - 1f));
                    float denominator = Mathf.Log(inner / (Mathf.Exp(_q) - 1f));
        ...
        ApplyCentralForce(Mass * acceleration * movementDirection);
    }
