...
public partial class HealthComponent : Control
{
    ...
    public float SetMaxHealth(float maxHealth)
    {
        ...
        if (maxHealth < _health) SetHealth(maxHealth);
    ...
    public float Hurt(float damage)
    {
        ...
        if (healthDelta < 0f) EmitSignal(SignalName.HealthChanged, this, healthDelta, excess);

        if (_health <= 0f) EmitSignal(SignalName.HealthFullyDepleted, this, -healthDelta, -excess);
    ...
    public float Heal(float heal)
    {
        ...
        if (0 != healthDelta) EmitSignal(SignalName.HealthChanged, this, healthDelta, excess);

        if (_maxHealth <= _health) EmitSignal(SignalName.HealthFullyRestored, this, healthDelta, excess);
